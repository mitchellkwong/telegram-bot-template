...
Note
----
Guess who's secretly an OOP fan,,, But also, python isn't java and shouldn't be treated as such!
...
class Feature(ABC):
    """Interface for implementing a simple feature.
    Requirements
    ------------
    help_text: str
    ...
    handler: telegram.ext.Handler
        A handler to match updates to the callback.
    ...
    @abstractmethod
    def handler(self):
        """Returns a handler to be registered with the dispatcher"""
        return
...
class Command(Feature):
    """Utility class to implement a simple command. A `telegram.ext.CommanHandler` is automatically constructed.
    Requirements
    ------------
    command: str
